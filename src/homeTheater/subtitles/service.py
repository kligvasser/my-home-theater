...
async def sweep_missing(config: AppConfig) -> SweepStats:
    ...
    if not (secrets.bazarr_url and secrets.bazarr_api_key):
        raise ValueError("BAZARR_URL / BAZARR_API_KEY are not set in .env.")
        ...
        async with httpx.AsyncClient(timeout=20.0) as http:
            ...
            for m in movies:
                ...
                try:
                    ...
                    stats.errors.append(f"movie {m.title}: {exc}")
            for e in episodes:
                ...
                try:
                    ...
                    stats.errors.append(f"episode {e.title}: {exc}")
        log.info("subtitle.done", **stats.as_dict())
    except Exception as exc:
        status = RunStatus.failed
        stats.errors.append(str(exc))
        log.error("subtitle.failed", error=str(exc))
    finally:
        with session_scope() as session:
